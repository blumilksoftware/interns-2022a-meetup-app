...
@section('content')
  <div class="container md:w-[800px] mx-auto">
    @auth
      <form action="{{ route('organizations.profiles.store', $organization) }}" method="post"
        ...
        <div>
          ...
          <div class="mt-6 flex flex-col gap-7">
              ...
              <div class="mt-1">
                <select name="label" id="label"
                  ...
                  @foreach ($availableProfiles as $availableProfile)
                    <option value="{{ $availableProfile['label'] }}" @if ($availableProfile['label'] === old('label')) selected @endif>
            ...
            <x-form-input id="link" label="Link" field="link" placeholder="Link" type="text" value="{{ old('link') }}" />
          </div>
        </div>
        <div class="pt-6">
